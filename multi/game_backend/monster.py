...
class monster :
    ...
    def pierre_tombale(self,map):
        if map[self._x,self._y] == "." or map[self._x,self._y] == "x":
            ret = True
            map[self._y][self._x] = "G"
            data = [{"i": f"{self._y}", "j":f"{self._x}", "content":"G"}]
            return data,ret
        else :
            print("je suis au mauvais endroit")
            ret = False
            data = []
            return data,ret
...
class bat(monster):
    ...
    def __init__(self,x_init,y_init):
        ...
        super().__init__("bat",'w',x_init,y_init,5,1,1,1)
