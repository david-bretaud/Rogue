...
window.addEventListener("DOMContentLoaded", (event) => {
    ...
    var player_id=location.pathname.split("/")[2];
    ...
    function dessiner_map(){
        /*Lit les spans contenant l'information*/
        var dat;
    ...
    var bruitage_potion = document.getElementById("bruitage_potion");
    var timer ; 
    ...
    function animation(num){
        ...
        */ 
        ...
        //timer = setTimeout(changeColor('rgb(233,220,164)'), 500);
        //timer = setTimeout(changeColor('rgb(100,100,100)'), 10000);
    }
    ...
    document.onkeydown = function(e){
        switch(e.keyCode){ // boutons appuyés
            case 37:
                socket.emit("move", {player:player_id, dx:-1, dy:0});
                break;
            case 38:
                socket.emit("move", {player:player_id, dx:0, dy:-1});
                break;
            case 39:
                socket.emit("move", {player:player_id, dx:1, dy:0});
                break;
            ...
            case 65: //A (on utilise Azerty tant que pas les numéros)
                socket.emit("use",{player: player_id, slot:1});
                bruitage_potion.play();
                break;
            case 90: //Z
                socket.emit("use",{player: player_id, slot:2});
                bruitage_potion.play();
                break;
            case 69: //E
                socket.emit("use",{player: player_id, slot:3});
                bruitage_potion.play();
                break;
            case 82: //R
                socket.emit("use",{player: player_id, slot:4});
                bruitage_potion.play();
                break;
            case 84: //T
                socket.emit("use",{player: player_id, slot:5});
                bruitage_potion.play();
                break;
            case 89: //Y
               socket.emit("use",{player: player_id, slot:6});
               bruitage_potion.play();
               break;
            case 85: //U
               socket.emit("use",{player: player_id, slot:7});
               bruitage_potion.play();
               break;
            case 73: //I
               socket.emit("use",{player: player_id, slot:8});
               bruitage_potion.play();
               break;
            case 81: //Q on utilise les touches du dessous pour looter
                socket.emit("loot",{player: player_id, slot:1});
                break;
            case 83: //S
                socket.emit("loot",{player: player_id, slot:2});
                break;
            case 68: //D
                socket.emit("loot",{player: player_id, slot:3});
                break;
            case 70: //F
                socket.emit("loot",{player: player_id, slot:4});
                break;
            case 71: //G
                socket.emit("loot",{player: player_id, slot:5});
                break;
            case 72: //H
                socket.emit("loot",{player: player_id, slot:6});
                break;
            case 74: //J
                socket.emit("loot",{player: player_id, slot:7});
                break;
            case 75: //K
                socket.emit("loot",{player: player_id, slot:8});
                break;
    ...
    socket.on("actualize_map", function(args){
        var player_num=args[0];
        var data = args[0];
        var arg1 = args[1];
        var arg2 = args[2];
        console.log(data);
        ...
        // ---------- TRACER DE LA NOUVELLE MAP SI BESOIN ----------
        if (arg1==true) {
            new_map(arg2);  
        }
        else{
