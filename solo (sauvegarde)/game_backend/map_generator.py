...
class Generator():
	#ajout de l'attribut nbr_gold : nombre de pièces d'or généré dans le niveau
    def __init__(self, width=64, height=64, max_rooms=15, min_room_xy=5, max_room_xy=10, rooms_overlap=False, random_connections=1,random_spurs=3, tiles=CHARACTER_TILES, nbr_gold = 25, nbr_potion=5):
        self.width = width
        self.height = height
        ...
        self.corridor_list = []
        self.tiles_level = []
        self.number_gold = nbr_gold
        self.number_potion = nbr_potion
    ...
    def gen_level(self):
        ...
        #add gold to the level:
        for i in range(self.number_gold):
            y_g = random.randint(0,(self.width -1))
            x_g = random.randint(0,(self.height -1))
            while self.level[x_g][y_g] != 'floor':
                x_g = random.randint(0,(self.height -1))
                y_g = random.randint(0,(self.width -1))
            self.level[x_g][y_g] = 'gold'

        # #add potions to the level (old version : now potion is derived from Item class):
        # for i in range(self.number_potion):
        #     y_g = random.randint(0,(self.width -1))
        #     x_g = random.randint(0,(self.height -1))
        #     while self.level[x_g][y_g] != 'floor':
        #         x_g = random.randint(0,(self.height -1))
        #         y_g = random.randint(0,(self.width -1))
        #     self.level[x_g][y_g] = 'potion'
    ...
    def gen_tiles_level(self):
        for row_num, row in enumerate(self.level):
            ...
            for col_num, col in enumerate(row):
                ...
                if col == 'gold':
                    tmp_tiles.append(self.tiles['gold'])
                # if col == 'potion':
